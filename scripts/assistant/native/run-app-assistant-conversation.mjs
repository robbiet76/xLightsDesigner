...
function buildAgentSystemPrompt(context = {}, userMessage = '') {
  ...
  return [
    ...
    'Only include actionRequest for bounded app-level actions that the app assistant should surface explicitly. Use generate_sequence_proposal only after the user has explicitly approved generating a reviewable proposal from saved native design intent. Use propose_display_metadata_from_layout only when the user asks to seed Display metadata from the current layout, and use apply_display_metadata_proposals only after the user explicitly approves promoting pending Display proposals. Do not use actionRequest for direct sequencing edits or silent navigation.',
