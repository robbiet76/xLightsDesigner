...
test('self-improvement cycle expands FSEQ reviews across multiple windows', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'xld-self-improve-fseq-windows-'));
  const geometryPath = path.join(root, 'geometry.json');
  const fseqPath = path.join(root, 'section.fseq');
  const manifestPath = path.join(root, 'manifest.json');
  writeJson(geometryPath, { artifactType: 'preview_scene_geometry_v1' });
        ...
        reviews: [
          {
            ...
            effectName: 'On',
            windows: [
              { id: 'intro', label: 'Intro', startMs: 0, endMs: 2000, creativeObjective: { coverage: 'sparse' } },
              { id: 'build', label: 'Build', startMs: 2000, endMs: 5000, creativeObjective: { coverage: 'wide' } },
              { id: 'release', label: 'Release', startMs: 5000, endMs: 8000, creativeObjective: { coverage: 'wide' } }
            ]
          }
        ]
      }
  ...
  const result = await runSelfImprovementCycle({
    ...
    buildFseqReview: ({ outDir, windowStartMs, windowEndMs, intent }) => {
      calls.push({ outDir, windowStartMs, windowEndMs, intent });
      const renderReviewPath = path.join(outDir, 'render-review.json');
  ...
  const phase = result.phases.find((row) => row.id === 'review_multi_window_fseq');
  assert.equal(phase.ok, true);
...
test('self-improvement cycle builds render-review revision objectives after review phases', async () => {
  ...
  const executionPhase = result.phases.find((row) => row.id === 'execute_revision_attempts');
