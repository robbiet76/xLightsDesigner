...
python3 "${ROOT_DIR}/tooling/merge-intent-maps.py" \
...
python3 "${ROOT_DIR}/evaluation/evaluate-priority-effect-selection.py" \
  --intent-map "${SELECTOR_MAP_OUT}" \
  --cases "${ROOT_DIR}/evaluation/fixtures/priority-effect-selection-cases.v4.json" \
  --out-file "${SELECTOR_EVAL_OUT}"

python3 "${ROOT_DIR}/generators/generate-current-effect-equalization-board.py" \
