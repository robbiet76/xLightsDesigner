...
for x in labels:
    if x in {'duty_factor','number_chases','chase_size','advances','fx_intensity','cycles'}:
  ...
  if bash "${SCRIPT_DIR}/run-packed-model-batch.sh" --manifest "${manifest}" --out-dir "${run_dir}" >>"${LOG_PATH}" 2>&1; then
    if [[ "${pack_id}" == *"range"* ]]; then
      ...
      if [[ -n "${param}" ]]; then
        ...
        python3 "${SCRIPT_DIR}/generate-parameter-region-summary.py" --run-dir "${run_dir}" --param "${param}" --out-file "${run_dir}/region-summary.json" >>"${LOG_PATH}" 2>&1 || status="failed"
      fi
    else
