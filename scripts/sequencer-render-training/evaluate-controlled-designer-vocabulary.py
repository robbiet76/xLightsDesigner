            ...
            for bucket in geometry_payload.get("intentBuckets", {}).values():
                for descriptor in bucket:
                    ...
                    structural_score, structural_missing = score_terms(
                        descriptor.get("structuralLabels", []), req_s, exc_s, 2
                    )
                    family_score, family_missing = score_terms(
                        descriptor.get("patternFamilies", []), req_f, exc_f, 3
                    )
...
def select_effect(candidates, top_n=12, top_matches=3):
    ...
    ranked = sorted(
        grouped.values(),
        key=lambda item: (-item["aggregateScore"], -item["matchCount"], item["effect"]),
    )
    return {
        "selectedEffect": ranked[0]["effect"] if ranked else None,
        "rankedEffects": [
            {**item, "topMatches": item["topMatches"][:top_matches]} for item in ranked
        ],
    }


def evaluate_case(intent_map, vocab, case):
    term_name = case["term"]
    term = vocab["terms"][term_name]
    candidates = collect_candidates(intent_map, term["query"])
    ...
    return {
        "caseId": case["caseId"],
        "term": term_name,
        "description": case.get("description") or term.get("description"),
        "query": term["query"],
...
def main():
    ...
    results = [evaluate_case(intent_map, vocab, case) for case in cases]
    payload = {
        "version": "1.0",
