...
test("build-effect-training-automation-plan classifies runnable and blocked effects", () => {
  ...
  assert.ok(plan.summary.runnableNowCount >= 2);
  assert.ok(plan.effects.some((row) => row.effectName === "Shockwave" && row.readiness === "ready_for_parameter_screening"));
  assert.ok(plan.effects.some((row) => row.effectName === "Twinkle" && row.readiness === "ready_for_parameter_screening"));
