...
struct LocalXLightsSessionService: XLightsSessionService {
    func loadSession(projectShowFolder: String) async throws -> XLightsSessionSnapshotModel {
        ...
        async let revision = readJSONAllowing404(from: "/sequence/revision")
        async let settings = readJSONAllowing404(from: "/sequence/settings")
    ...
    private func mutateShowDirectory(
        ...
        let confirmed = normalizePath(string(data["showDirectory"]))
        guard confirmed == normalizedPath else {
