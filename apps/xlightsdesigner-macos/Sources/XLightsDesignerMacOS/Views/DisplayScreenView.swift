...
struct DisplayScreenView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    case let .selected(entry):
                        ...
                        if !entry.targetBehaviorFacts.isEmpty {
                            targetBehaviorFactsSection(entry.targetBehaviorFacts)
                        }
    ...
    private var filterRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                ...
                Button("Propose From Display") {
                    model.proposeMetadataFromDisplay()
                }
    ...
    private var discoveryProposalSheet: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("Review Proposed Metadata")
                .font(.title2)
                .fontWeight(.semibold)
            Text("These proposals came from display discovery or the live xLights display. Applying them promotes the reviewed metadata into the active display label store and maps it to the linked xLights models.")
                .foregroundStyle(.secondary)
    ...
    private func targetBehaviorFactsSection(_ facts: [DisplayTargetBehaviorFactModel]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVStack(alignment: .leading, spacing: 8) {
                ForEach(facts.prefix(6)) { fact in
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(alignment: .firstTextBaseline) {
                            Text(fact.effectName.isEmpty ? "Effect" : fact.effectName)
                                .font(.subheadline)
                                .fontWeight(.medium)
                            Spacer()
                            Text(fact.outcomeSummary)
                                .font(.caption)
            ...
            if facts.count > 6 {
                Text("+ \(facts.count - 6) more learned records")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
