...
test("sequencer_designer requires intent handoff", () => {
  assert.throws(
    () => buildSequencerDesignerPlan({ analysisHandoff: sampleAnalysis(), intentHandoff: null }),
    /intent_handoff_v1 is required/i
  );
});

test("sequencer_designer builds validated command plan from handoffs", () => {
  const out = buildSequencerDesignerPlan({
...
test("sequencer_designer emits reduced-confidence warning when analysis is missing", () => {
  const out = buildSequencerDesignerPlan({
