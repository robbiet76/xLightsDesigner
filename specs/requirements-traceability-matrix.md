...
| XSC-001 | `sequence-agent/xlights-api.md` | owned API on `49915`, sequence/timing/effects/layout/render-feedback contracts | owned API validation artifact in `_xlightsdesigner_api_validation/2026-04-24T01-13-58-788Z/` + `scripts/sequencer-render-training/runners/run-stage1-coverage-chunked.sh` |
...
| UI-001 | `app-ui/native-app.md` | native application shell, workflow UX, team chat presentation | `swift test --package-path apps/xlightsdesigner-macos` |
| XSC-002 | `designer-dialog/designer-interaction-contract.md` | creative design conversation, proposal shaping, intent handoff | `scripts/native/validate-metadata-tag-proposal-flow.mjs`, `scripts/native/run-full-handoff-validation.mjs`, `scripts/native/validate-active-target-sync.mjs` |
| APP-002 | `local-completion-roadmap.md` | native Design -> Sequence -> Review -> History flow with backup/restore and session recovery | `ReviewScreenViewModelTests.swift`, `XLightsSessionViewModelTests.swift`, native package tests |
