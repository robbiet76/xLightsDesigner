...
5. Add conversational board revision controls and fixture validation for edit lineage. Done: the native generator supports `revisionRequest`, loads the current board image, calls the provider edit path, appends `board-r###` lineage, and writes the revised manifest/files under the same app-owned artifact folder. The macOS Design screen now exposes a revision request field and explicit revise action.
