...
`Shockwave` has now completed its broad-geometry screening pass, is structurally retrievable for constrained requests, and is selector-ready within the current merged selector set. It should still stay out of broader selector claims until it is calibrated against more effects.
