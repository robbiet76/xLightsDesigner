...
## 9) Storage Direction
Current storage direction is:
- keep all xLightsDesigner metadata under the app-owned root,
- keep shared analysis under `library/tracks/`,
- keep sequence-specific metadata under `sequencing/sequences/<sequence-id>/sequence.xdmeta`,
- leave `showFolderPath` and `mediaPath` as references inside the project file.

Initial development policy:
- do not add compatibility readers for old sidecar locations
- do not preserve parallel storage paths
- do not maintain old and new metadata locations at the same time
- if stale metadata roots or stale sidecar paths exist, remove them after backup rather than building runtime fallback logic
...
4. write sequence metadata only under the app-root sequencing store,
5. write reusable analysis only to the shared library storage,
6. delete stale alternate metadata roots instead of reading from them.

Long-term:
1. keep reusable track analysis in the shared app library,
2. keep project-specific sequencing state in app-root project metadata,
3. do not reintroduce alternate storage paths or compatibility fallbacks.
